...
class AnalysisWeb extends StatefulWidget {
  const AnalysisWeb({super.key});

  @override
  State<AnalysisWeb> createState() => _AnalysisWebState();
}

class _AnalysisWebState extends State<AnalysisWeb> {
  DateTimeRange? selectedRange;
  Map<String, int> categoryCounts = {};
  bool isLoading = false;
  bool showLineChart = false; // 🔄 İleri/geri görünümler arası geçiş
  final Set<String> usageDaySet = {}; // 📅 Kullanılan ajanda günleri

  int usageDayCount = 0;
  int totalDayCount = 0;
  int _agendaViewIndex = 0;
  ...
  Future<Map<String, int>> getDailyTaskCounts(DateTimeRange range) async {
  ...
  Future<void> _fetchAgendaUsage() async {
    ...
    for (int i = 0; i < totalDayCount; i++) {
      ...
      if (found) {
        usageDayCount++;
        usageDaySet.add(dayPrefix); // 🔹 Kullanılan günleri set'e ekle
      }
    }
  ...
  Widget _buildPieChartCard() {
    ...
    categoryCounts.forEach((key, value) {
      ...
      final color = kategoriRenkleri[key] ?? Colors.grey;
                        ...
                        return Row(
                          ...
                          children: [
                            Container(
                              width: 12,
                              height: 12,
                              color: kategoriRenkleri[key] ?? Colors.grey,
                            ),
  ...
  final Map<String, Color> kategoriRenkleri = {
    'Job': Color(0xFF9C27B0), // Mor
    'School': Color(0xFF3F51B5), // Lacivert
    'Shopping': Color(0xFFFFA726), // Turuncu
    'Health': Color(0xFF66BB6A), // Yeşil
    'Personal': Color(0xFFEC407A), // Pembe
    'Sport': Color(0xFF26C6DA), // Açık Mavi
  };
